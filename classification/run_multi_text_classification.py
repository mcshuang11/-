...
def main():
    ...
    if "train" in datasets:
        ...
        label_list = []
        for s in datasets["train"]["label"]:
            label_list.extend(s.split(","))
    ...
    if "train" in datasets:
        ...
        elif config.label2id == PretrainedConfig(num_labels=num_labels).label2id:
            label_to_id = {v: i for i, v in enumerate(label_list)}
        else:
            label_to_id = None
    ...
    if "validation" in datasets and config.label2id is not None:
        ...
        for v in datasets["validation"]["label"]:
            validation_label_list.extend(v.split(","))
        validation_label_list = list(set(validation_label_list))
    ...
    def preprocess_function(examples):
        ...
        if config.label2id is not None and "label" in examples:
            result["label"] = [[config.label2id[l] for l in label.split(",")] for label in examples["label"]]
        result["label"] = mlb.fit_transform(result["label"]).tolist()
        return result
    ...
    def compute_metrics(labels, preds, target_names):
        ...
        acc = accuracy_score(tf.reshape(labels, [-1]), tf.reshape(preds, [-1]))
    ...
    with training_args.strategy.scope():
        ...
        model.compile(optimizer=optimizer, loss=loss_fn, metrics=[accu])
        # endregion
        ...
        if tf_data["test"] is not None:
            ...
            with open(output_test_file, "w") as writer:
                ...
                for label in predicted_class.tolist():
